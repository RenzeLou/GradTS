...
def train_config(parser):
    ...
    parser.add_argument("--instance_threshold", type=str,
                        default="0.62,0.67")  # in subsampling exp, different auxiliary tasks may have different thres
...
def generate_copy_subset(ori_data_dir: str, data_dir: str, need_file: str, corr_path: str, ins_thres: float):
    ...
    #     origin = json.load(rf)
    if type(ins_thres) == str:
        ins_thres = float(ins_thres)
    ori_data = []
    ori_data_cnt = 0
...
def main():
    ...
    if args.method == 1:
        ...
        if not os.path.exists(output_dir):
            os.mkdir(output_dir)
        output_dir = os.path.join(output_dir, str(args.instance_threshold.replace(",", "_")))
        if not os.path.exists(output_dir):
            os.mkdir(output_dir)
        print("save checkpoint to {}".format(output_dir))
        # deal with subset
        ...
        if not os.path.exists(data_dir):
            os.mkdir(data_dir)
            is_exist = False
        thresholds = args.instance_threshold.split(",")
        data_dir_remain = copy.deepcopy(data_dir)
        data_dirs = []
        for threshold in thresholds:
            data_dir = os.path.join(data_dir_remain, threshold)
            ...
            if not is_exist:
                print("the whole path {} had yet been launched".format(data_dir))
            print("accumulate the subset of main task {} at {}".format(args.main_task, data_dir))
            # find all required json file
            exist_file = FindAllSuffix(data_dir, ".json")
            need_file_lis = CheckRemainFile(candidate_task, exist_file)
            # generate all required training subset, copy all required test\dev set
            print("\ngenerating required subset begin...")
            for need_file in need_file_lis:
                generate_copy_subset(ori_data_dir, data_dir, need_file, corr_path, threshold)
            print("generating required subset end...\n")
    else:
    ...
    train_datasets = []
    assert len(args.train_datasets) == len(data_dirs) + 1
    for i, dataset in enumerate(args.train_datasets):
        prefix = dataset.split('_')[0]
        if prefix in tasks:
            continue
        task_id = len(tasks)
        ...
        if dataset == args.main_task and args.method == 3:
            train_path = os.path.join(ori_data_dir, '{}_train.json'.format(dataset))
        else:
    ...
    for dataset in args.test_datasets:
        ...
        if (dataset == args.main_task and args.method == 3) or (dataset in ["mnli_matched","mnli_mismatched"]):
            dev_path = os.path.join(ori_data_dir, '{}_dev.json'.format(dataset))
        else:
            raise RuntimeError("can not do this!")
            dev_path = os.path.join(data_dir, '{}_dev.json'.format(dataset))
        dev_data = None
        ...
        if (dataset == args.main_task and args.method == 3) or (dataset in ["mnli_matched","mnli_mismatched"]):
            test_path = os.path.join(ori_data_dir, '{}_test.json'.format(dataset))
        else:
            test_path = os.path.join(data_dir, '{}_test.json'.format(dataset))
        test_data = None
