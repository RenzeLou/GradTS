...
def train_config(parser):
    ...
    # cross validation
    parser.add_argument("--k_fold", type=int, required=True, help="N for K Fold")
    parser.add_argument("--N", type=int, required=True, help="which part of data used for validation")
...
def generate_cross_validation(train_file: str, k_fold: int, N: int, out_dir: str, main_task: str, move_test_file=True,
                              verbose=True):
    all_train_ins = []
    for line in open(train_file, "r"):
        all_train_ins.append(json.loads(line))

    all_num = len(all_train_ins)
    per_num = all_num // k_fold
    validation_part = all_train_ins[N * per_num: (N + 1) * per_num]
    train_part = all_train_ins[0:N * per_num] + all_train_ins[(N + 1) * per_num:]

    train_out = os.path.join(out_dir, "{}_train.json".format(main_task))
    eval_out = os.path.join(out_dir, "{}_dev.json".format(main_task))
    test_out = os.path.join(out_dir, "{}_test.json".format(main_task))

    with open(train_out, "w") as t:
        # json.dump(train_part, t)
        for train_ins in train_part:
            t.write("{}\n".format(json.dumps(train_ins)))
    with open(eval_out, "w") as d:
        # json.dump(validation_part, d)
        for eval_ins in validation_part:
            d.write("{}\n".format(json.dumps(eval_ins)))
    if move_test_file:
        shutil.copyfile(train_file.replace("train", "test"), test_out)
    if not verbose:
        print("total ori train:", all_num)
        print("cv train:", len(train_part))
        print("cv eval:", len(validation_part))
        print("cv train path:", train_out)
        print("cv eval path:", eval_out)
        print("cv test path:", test_out)
...
def main():
    ...
    ori_data_dir = args.data_dir
    main_data_dir = os.path.join(ori_data_dir, "cv_{}_{}".format(args.k_fold, args.N))
    if not os.path.exists(main_data_dir):
        os.makedirs(main_data_dir, exist_ok=True)

    # split ori train data to generate cross validation partition
    assert args.N in list(range(0, args.k_fold))
    pass_in = {"train_file": os.path.join(ori_data_dir, "{}_train.json".format(args.main_task)),
               "k_fold": args.k_fold,
               "N": args.N,
               "out_dir": main_data_dir,
               "main_task": args.main_task,
               "verbose": False}
    generate_cross_validation(**pass_in)
    # exit()
    ...
    if args.method == 1:
        ...
        if len(args.train_datasets) == 2:
            ...
            output_dir = os.path.join(output_dir,
                                      "_".join(args.train_datasets) + "_{}_{}".format(args.k_fold, args.N))
            if not os.path.exists(output_dir):
                os.mkdir(output_dir)
            print("save checkpoint to {}".format(output_dir))
        else:
    ...
    train_datasets = []
    for order, dataset in enumerate(args.train_datasets):
        data_dir = main_data_dir if order == 0 else ori_data_dir
