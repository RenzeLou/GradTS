...
''' train with mix ratios and don't fix seed, generate new data sampler each epoch , to guarantee sample evenly  '''
import argparse
import json
import os
...
def data_config(parser):
    ...
    parser.add_argument('--train_datasets', default='cola,mrpc,rte')
...
def train_config(parser):
    ...
    # mix_ratio
    parser.add_argument('--mix_ratios', type=str, default="parallel",
                        help="the mix_training ratio, like 9,3,2; parallel indicate 1:1:1")
...
def main():
    # mix ratio ==========================================
    mix_ratio = []
    if args.mix_ratios == "parallel":
        mix_ratio = [-1000] * len(args.train_datasets)
    else:
        mix_ratio = args.mix_ratios.split(",")
        assert len(mix_ratio) == len(args.train_datasets)
    mix_ratio = [int(t) for t in mix_ratio]
    print("mix_ratio is ", mix_ratio)
    # we only eval,test on the mian task
    assert len(args.test_datasets) == 1 or 'mnli' in args.train_datasets[0], "we eval,test all the exp on the main task but MNLI"
    # ====================================================
    # set up dist
    ...
    # construct a list which contain all single task train data loaders============================
    # we don't apply multi gpu here
    ...
    for train_dataset in train_datasets:
        sig_task_train_dataset = train_dataset
        # sig_task_batch_sampler = MultiTaskBatchSampler([train_dataset], args.batch_size, args.mix_opt, args.ratio,
        #                                                bin_on=args.bin_on, bin_size=args.bin_size,
        #                                                bin_grow_ratio=args.bin_grow_ratio)
        sampler = RandomSampler(sig_task_train_dataset)
        sig_task_train_data = DataLoader(sig_task_train_dataset, sampler=sampler,batch_size=args.batch_size,
                                         collate_fn=train_collater.collate_fn, pin_memory=args.cuda)
        mix_training_data.append(sig_task_train_data)


    mix_data_len = [len(t) for t in mix_training_data]
    mix_data_len = sum(mix_data_len)
    num_all_batches = args.epochs * mix_data_len
    ...
    for epoch in range(0, args.epochs):
        ...
        # training on the whole main task data, record the training batch num, and then pass the auxiliary data according to the auxiliary batch num
        print("=" * 10 + "main task training begin!" + "=" * 10)
        ### each epoch generate new dataloader for each task
        mix_training_data = []
        for train_dataset in train_datasets:
            sig_task_train_dataset = train_dataset
            # sig_task_batch_sampler = MultiTaskBatchSampler([train_dataset], args.batch_size, args.mix_opt, args.ratio,
            ...
            sig_task_train_data = DataLoader(sig_task_train_dataset, sampler=sampler,batch_size=args.batch_size,
                                         collate_fn=train_collater.collate_fn, pin_memory=args.cuda)
            mix_training_data.append(sig_task_train_data)
        print("type of mix_training_data[0]",type(mix_training_data[0]))
        # sg_dl = mix_training_data[0]
        # assert isinstance(DataLoader,sg_dl)
        for i, (batch_meta, batch_data) in enumerate(mix_training_data[0]):
            ...
            if (model.updates) % (args.log_per_updates) == 0 or model.updates == 1:
                ramaining_time = \
                    str((datetime.now() - start) / (i + 1) * (len(mix_training_data[0]) - i - 1)).split('.')[0]
        ...
        main_batch_num = i + 1
        mix_ratio_percent = np.array(mix_ratio) / mix_ratio[0]
        # mix_ratio_percent = mix_ratio_percent.tolist()
        auxiliary_batch_num = (mix_ratio_percent * main_batch_num).tolist()
        auxiliary_batch_num = [int(t) for t in auxiliary_batch_num]
        auxiliary_batch_num = auxiliary_batch_num[1:]
        print("====> training auxilary task data batch num is {}".format(auxiliary_batch_num))
        parallel = True if mix_ratio[0] == -1000 else False
        if not parallel:
            for n, task_train_data in enumerate(mix_training_data[1:]):
                # for each auxiliary task, training them with reference to the main batch num
                auxiliary_train_num = auxiliary_batch_num[n]
                re_run = True
                has_run = 0
                while re_run:
                    for id, (batch_meta, batch_data) in enumerate(task_train_data):
                        batch_meta, batch_data = Collater.patch_data(device, batch_meta, batch_data)
                        task_id = batch_meta['task_id']
                        model.update(batch_meta, batch_data)
                        has_run += 1
                        if has_run >= auxiliary_train_num:
                            print("-" * 10 + "auxiliary task {} training end!".format(n + 1) + "-" * 10)
                            re_run = False
                            break
                    if re_run:
                        print("this auxiliary task has yet ran with given batch num, re run!")
        else:
            for n, task_train_data in enumerate(mix_training_data[1:]):
                for id, (batch_meta, batch_data) in enumerate(task_train_data):
                    batch_meta, batch_data = Collater.patch_data(device, batch_meta, batch_data)
                    task_id = batch_meta['task_id']
                    model.update(batch_meta, batch_data)
                print("auxiliary task {} parallel training end!".format(n+1))
            print("whole auxiliary task parallel training end!")
