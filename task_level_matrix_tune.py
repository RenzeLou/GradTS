...
# os.environ['CUDA_VISIBLE_DEVICES'] = '8'  # TODO: be careful if use to debug
...
from mt_dnn.inference import eval_model, extract_encoding, get_head_matrix
...
def tune_get_head_matrix(model, datasets, data_list, task_defs, save_path, device=None, logger=None):
    ...
    for idx, dataset in enumerate(datasets):
        ...
        get_head_matrix(model, test_data, device=device, save_path=save_path)  # note this is training data
...
def main():
    ...
    # save_name = os.path.join(save_path, "gradient_abs.pkl")
    save_name = os.path.join(save_path, "gradient_layer_normal_task.pkl")
    if os.path.isfile(save_name) and not args.force:
        raise RuntimeError("there is already has gradient file {}\nPlease use '--force' to overcome".format(save_name))
    # ======================================================
    ...
    for dataset in args.test_datasets:
        ...
        dev_path = os.path.join(data_dir, '{}_train.json'.format(dataset))  # Note!!!! it is train set
        dev_data = None
        if os.path.exists(dev_path):
            ...
            dev_data = DataLoader(dev_data_set, batch_size=args.batch_size_eval, sampler=sampler,
    ...
    for epoch in range(0, args.epochs):
        ...
        for i, (batch_meta, batch_data) in enumerate(multi_task_train_data):
            ...
            #                n_updates=args.save_per_updates, with_label=False, tensorboard=tensorboard,
        ...
        #            logger=logger)
        # evaluation(model, args.test_datasets, test_data_list, task_defs, output_dir, epoch, with_label=False,
        ...
        # we don't save model here
        # if args.local_rank in [-1, 0]:
        #     model_file = os.path.join(output_dir, 'model_{}.pt'.format(epoch))
        #     model.save(model_file)
    tune_get_head_matrix(model, args.test_datasets, test_data_list, task_defs, save_path=save_name, device=device,
                         logger=logger)  # note it is train set

    end = time.time()
    run_time = (end - begin) / 60.

    with open(os.path.join(save_path, "task_time.txt"), "w") as f:
        f.write("task time consumption: %.3f minutes" % run_time)
    if args.tensorboard:
        tensorboard.close()
