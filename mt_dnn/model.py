...
class MTDNNModel(object):
    ...
    def update_no_grad(self, batch_meta, batch_data, attention_gradients, nb_eval_steps):
        ''' return [12,12] absolute attention gradient dict with reference to a batch of data '''
        self.network.train()
        # y = batch_data[batch_meta['label']]
        try:
            y = torch.LongTensor(batch_meta['label'])
        except:
            print("\n=============\nthere is an error!")
            print("batch_meta['label']",batch_meta['label'])   ### TODO: note this, this mainly due to the second dim of batch_meta['label'] are not at consensus, one solution is set batch size to 1
            # y = batch_data[batch_meta['label']]
            raise RuntimeError("Oh no, please set batch size to 1")
        y = self._to_cuda(y) if self.config['cuda'] else y
        ...
        # scale loss
        # loss = loss / self.config.get('grad_accumulation_step', 1)
        # TODO: get the gradient of this batch without updating parameters
        num_attention_heads = self.network.num_attention_heads
        ...
        for n, p in model.named_parameters():
            if "attention.self" in n and "weight" in n:
                ...
                gradient = np.abs(gradient.cpu().numpy())  # absolute it.
        ...
        return attention_gradients, nb_eval_steps
