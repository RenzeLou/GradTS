...
# trial_num = 1
bert_model_type = "bert-base-cased"  # bert-base-cased,bert-base-uncased,roberta-base,bert-large-uncased,roberta-large,bert-base-cased,bert-large-cased  ## TODO
...
python_script = 'train_with_corr_cv.py'
...
for task in ["sc"]:
    ...
    for order, t in enumerate(sorted_candidate):
        train_datasets.append(t)
        if order == len(sorted_candidate) - 1:
            for N in range(k_fold):
                ...
                template += '--data_dir ${DATA_DIR} --encoder_type ${ENCODER_TYPE} --num_hidden_layers ${NUM_LAYER} ' \
                            ...
                            '--learning_rate ${lr} '
                template += "--k_fold {} --N {} ".format(k_fold, N)
