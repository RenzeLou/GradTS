        ...
        if test_data is not None:
            ...
            for key, val in test_metrics.items():
                ...
                if isinstance(val, str):
                    print_message(logger, 'Task {} -- {} {} -- {} {}: {}'.format(dataset, updates_str, updates,
...
def main():
    ...
    if args.method == 1:
            ...
            if not os.path.exists(latest_file):
                ...
                if not run_flag:
                    ...
                    if os.path.isfile(current_continue):
                        warnings.warn("the 'whether_continue_flag' has existed in current direction,modify it to False to avoid any breaks!")
        ...
        if not os.path.exists(output_dir):
            os.mkdir(output_dir)
        output_dir = os.path.join(output_dir, str(args.instance_threshold))
        if not os.path.exists(output_dir):
            os.mkdir(output_dir)
        print("save checkpoint to {}".format(output_dir))
        # deal with subset
        ...
        for need_file in need_file_lis:
            generate_copy_subset(ori_data_dir, data_dir, need_file, corr_path, args.instance_threshold)
    ...
    for dataset in args.test_datasets:
        ...
        # print("os.path.exists(dev_path)",os.path.exists(dev_path))
