...
# set args
method = 1  # 1,2,3
task_thresholds = [0.4, 0.55, 0.6]  # 0.66,0.59,0.69,0.59,0.57   ## TODO: MOD
bert_model_type = "roberta-large"  # bert-base-uncased,roberta-base,bert-large-uncased，roberta-large,bert-base-cased,bert-large-cased  ## TODO
main_task = "pos"
...
lr = '2e-5'  ## TODO:maybe MOD
wnli_only = True  # due to the instability of wnli, run the wnli seperately
...
# for wnli
# ===========================================
for task_threshold in task_thresholds:
    wnli_only = True
    for t in ["wnli"]:
        main_task = t
        candidate_set = ",".join([t for t in task_list if t != main_task])  # imply the exp type
...
shell_name = "wnli_{}_{}_{}_{}.sh".format(method_dict[method], len(task_list), bert_model_type,
                                          "_".join(
                                              list(map(str, task_thresholds)))) if wnli_only else "{}_{}_{}.sh".format(
    method_dict[method],
    len(task_list),
    bert_model_type)
